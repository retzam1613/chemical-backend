...
def get_all_approvals():
    ...
    cursor.execute("""
        ...
        UNION ALL
        SELECT 'EXPORT' as type, r.id, COALESCE(r.order_code, '#' || r.id) as order_code, COALESCE(u.full_name, 'ไม่ระบุ') as requester_name, c.name as chemical_name, c.brand, r.requested_quantity as qty, c.package_unit as unit, COALESCE(r.status, 'PENDING') as status, r.remark, TO_CHAR(COALESCE(r.created_at, CURRENT_TIMESTAMP), 'DD/MM/YYYY HH24:MI') as created_at_str
        FROM requisitions r LEFT JOIN users u ON r.user_id = u.id JOIN chemicals c ON r.chemical_id = c.id WHERE r.status = 'PENDING' OR r.status IS NULL
        ORDER BY id DESC;
    """)
